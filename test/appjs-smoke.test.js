...
test('buildAddPayload produces a well-formed schedule:add payload', () => {
  const p = buildAddPayload({
    ...
    contentItemGuid: 'ci', scheduleGuid: 'sg', stationName: 'Connect', title: 'Evening Ride',
  });
  ...
  assert.equal(p.stationName, 'Connect');
...
test('buildAddPayload coerces/guards missing optionals', () => {
  ...
  assert.equal(p.vertical, false); assert.equal(p.contentItemGuid, ''); assert.equal(p.stationName, '');
});
